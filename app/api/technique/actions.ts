...
export async function createTechnique(technique: Omit<TechniqueType, "id">) {
  try {
    const [newTechnique] = await sql<TechniqueType[]>`
        INSERT INTO technique (title, photo_path, bucket_volume, max_depth, weight, load_capacity_auto, load_capacity_arrow, boom_reach, side_length, price, id_categories)
        VALUES (${technique.title}, ${technique.photo_path} ${technique.bucket_volume}, ${technique.max_depth}, ${technique.weight}, ${technique.load_capacity_auto}, ${technique.load_capacity_arrow}, ${technique.boom_reach}, ${technique.side_length}, ${technique.price}, ${technique.id_categories})
        RETURNING *
      `;
    return newTechnique;
  } catch (error) {
